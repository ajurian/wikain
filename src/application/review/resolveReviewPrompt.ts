...
export async function resolveReviewPrompt(
  ...
  if (card === undefined) {
    throw new Error(
      `resolveReviewPrompt: no card for user ${userId} / sense ${senseId}`,
    );
  }
  ...
  if (item === undefined) {
    throw new Error(
      `resolveReviewPrompt: no catalog item for sense ${senseId}`,
    );
  }
  ...
  switch (tier) {
    ...
    case "free":
      ...
      return {
        ...
        pos,
        lemma: item.lemma,
        intendedSense: item.productive_meaning, // use productive_meaning for now - intended senses generated by LLM are too long
